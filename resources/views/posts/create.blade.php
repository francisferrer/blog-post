<x-layouts.app :title="__('Create Post')">
    ...
    <form action="{{ route('posts.store') }}" method="POST" class="mx-auto mt-16 max-w-xl sm:mt-20">
            ...
            <div class="sm:col-span-2">
                ...
                <div class="mt-2.5">
                    <textarea id="content" name="content" rows="4"
                        class="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500">{{ old('content') }}</textarea>
                </div>
        ...
        <div class="mt-10">
            <button type="submit"
                class="block w-full rounded-md bg-indigo-500 px-3.5 py-2.5 text-center text-sm font-semibold text-white shadow-xs hover:bg-indigo-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">Create
                post</button>
        </div>
    </form>
</x-layouts.app>
